...
try {
    $stmt = $pdo->prepare("UPDATE information_generale SET statut_bien = 'publie' WHERE id_bien = ? LIMIT 1");
    $stmt->execute([$id]);
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if ($owner) {
            $titre = 'Bien validé';
            $message = 'Votre annonce a été validée par l\'administration.';
        ...
        echo json_encode(['success' => false, 'message' => 'Bien introuvable ou déjà publié']);
    }
    exit;
} catch (PDOException $e) {
    http_response_code(500);
    error_log('validate_bien.php PDO ERROR: ' . $e->getMessage());
