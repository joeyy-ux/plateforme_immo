...
// Le frontend envoie : payload.append("data", JSON.stringify(...))
if (!isset($_POST['data'])) {
...
$response = [
    'success' => false,   // passera à true à la fin
    'message' => '',
    'errors'  => [],      // erreurs par champ
    'step'    => null     // étape à revenir si erreur
];
...
// Photo principale
$photo_principale = $_FILES['photo_principale'] ?? null;

// Photos des pièces (tableau)
$pieces_photos = $_FILES['pieces_photos'] ?? null;

/*
À ce stade :
- $photo_principale contient UN fichier (ou null)
- $pieces_photos contient les photos par pièce (ou null)

👉 On ne traite PAS encore les fichiers ici
👉 On vérifiera ça à l’étape 8
*/
...
/* =========================
   ...
   (ex: "Salon principal" → salon_principal)
========================= */
...
/* =========================
   PHOTOS PAR PIÈCE (1 DOSSIER / PIÈCE)
========================= */
...
foreach ($pieces as $index => $piece) {
    ...
    $piecesSaved[] = [
        ...
        'dossier'=> 'uploads/publieBien/pieces/' . $pieceFolderName,
        'photos' => $savedPhotos
    ];
}
...
/*
À CE STADE :
✔ $photoPrincipalePath → chemin photo principale
✔ $piecesSaved → pièces + dossiers + photos
✔ $videoPlatform / $videoUrl valides

👉 Ces données seront utilisées à l’ÉTAPE 9 (insertion BDD)
*/
...
$stmt = $pdo->prepare("
    INSERT INTO information_generale (
        id_utilisateur, type_bien, type_offre, statut, meuble,
...
$stmt = $pdo->prepare("
    INSERT INTO localisation_bien (id_bien, ville, commune, quartier)
    VALUES (?, ?, ?, ?)
");
...
foreach (array_merge($accessibiliteListe, $accessibiliteAutres) as $acc) {
    $stmt->execute([$id_bien, $acc]);
}
...
foreach (array_merge($commoditesListe, $commoditesAutres) as $com) {
    $stmt->execute([$id_bien, $com]);
}
...
foreach ($conditionsPaiement as $cond) {
    $stmtCond->execute([$id_bien, $cond]);
}
...
foreach ($bonusListe as $b) {
    $stmtBonus->execute([$id_bien, $b]);
}
...
$pdo->commit(); echo json_encode([ 'success' => true, 'message' => 'Annonce publiée avec succès', 'id_bien' => $id_bien ]); exit; } 
